          ...
          <li class="post">
            <div class="content">
                ...
                <p v-if="post.category" class="ui label">
                  ...
                  <button @click="likepost(index)" class="ui red right floated button">いいね {{ post.like }}</button>
                  <button @click="sympathypost(index)" class="ui blue right floated button">わかる {{ post.sympathy }}</button>
              　</div>
...
export default {
  ...
  methods: {
    ...
    likepost(index) {
      this.posts[index].like = Number(this.posts[index].like) || 0;
      this.posts[index].like++;
    },
    sympathypost(index) {
      // わかるのカウントを増やす
      this.posts[index].sympathy = Number(this.posts[index].sympathy) || 0;
      this.posts[index].sympathy++;
    },
